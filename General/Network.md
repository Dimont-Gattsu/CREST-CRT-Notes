...
### One-liner for Host Discovery, TCP & UDP script scans
```
NETWORK="10.0.0.0/24"
mkdir nmap && cd nmap

nmap -sn "$NETWORK" -oG - | awk '/Status: Up/{print $2}' > live_targets.txt && nmap -sC -sV -Pn -T4 --open -oA tcp-open -iL live_targets.txt && nmap -sU -Pn -n --open -oA udp-open -iL live_targets.txt
```
...
### nexthop
Used to specify multiple next‑hop gateways (usually for load‑balancing or redundancy) for a destination:
```
ip route add <DEST> nexthop via <IP1> nexthop via <IP2>...
```
